...
New publication intake starts with a `.docx` or text-based `.pdf` using the institutional article template. The private object is stored in the existing `ResearchFileData` bucket under the authenticated researcher's UUID folder. `publications.document_path`, `document_name` and `document_mime_type` identify that object; `document_metadata` stores the validated, user-corrected optional template fields. Title and abstract remain first-class required columns. Legacy publications may have no document. Document identity and extracted metadata are immutable after submission. Storage reads are limited to the owner/admin until the associated publication is published; public publication rows remain readable without login. Image-only scanned PDFs require OCR before upload and are rejected when no extractable text is available.
